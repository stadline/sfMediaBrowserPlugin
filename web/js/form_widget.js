...
function addViewOnClick(tag, target) {
	...
	tag.onclick = function() {
		href = target.value;
		if (href) window.open(href, 'sfMediaBrowserViewFile', '');
	};
}
...
HTMLElement.prototype.getElementsByClassName = function(class_name) {
	...
	for (i; i < tags.length; ++i) {
		tag = tags[i];
		if (tag.hasAttribute('class') && tag.getAttribute('class') == class_name) matches.push(tag);
	}
	return matches;
};
